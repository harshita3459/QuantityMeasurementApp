...
import com.app.quantityservice.exception.QmaException;
import com.app.quantityservice.repository.QuantityOperationRepository;
...
public class QuantityServiceImpl implements IQuantityService {
    ...
    // ── Spring injects this via constructor — NO @RequiredArgsConstructor ─
    ...
    // ═════════════════════════════════════════════════════════════════════
    //  UNIT CONVERSION HELPERS — business logic preserved exactly
    // ═════════════════════════════════════════════════════════════════════
    ...
    private void checkSameType(QuantityInputDTO in) {
        String type1 = in.getThisQuantity().getMeasurementType();
        String type2 = in.getThatQuantity().getMeasurementType();
    ...
    private QuantityOperation saveOp(String op, QuantityInputDTO in, double result) {
        ...
        entity.setMeasurementType(in.getThisQuantity().getMeasurementType());
    ...
    private void validateInput(QuantityInputDTO in) {
        if (in == null ||
            ...
            throw new QmaException("Invalid input: missing fields");
        }
    }
    ...
    public OperationResponse add(QuantityInputDTO in) {
    	validateInput(in);
        checkSameType(in);
        String type = in.getThisQuantity().getMeasurementType();
    ...
    public OperationResponse convert(QuantityInputDTO in) {
        ...
        QuantityOperation op = saveOp("CONVERT", in, result);
